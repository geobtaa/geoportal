        ...
        if (!language) {
            ...
            for (var i = 0; i < language.length; i++) {
                ...
                if (typeof value === "string") {
                    t = new internal_1.Language(value, defaultLocale);
                }
                else {
                    t = new internal_1.Language(value["@value"], value["@language"] || defaultLocale);
                }
                tc.push(t);
            }
        }
            ...
            t = new internal_1.Language(language, defaultLocale);
            tc.push(t);
            return tc;
        }
        else {
            ...
            if (language["@value"]) {
                // presentation 2
                t = new internal_1.Language(language["@value"], language["@language"] || defaultLocale);
                tc.push(t);
            }
            else {
                // presentation 3
                Object.keys(language).forEach(function (key) {
                    ...
                    if (language[key].length) {
                        t = new internal_1.Language(language[key], key);
                        tc.push(t);
                    }
                    else {
            ...
            if (locale) {
                var language = languageCollection.filter(function (t) {
                    return t.locale === locale ||
                        internal_1.Utils.getInexactLocale(t.locale) === internal_1.Utils.getInexactLocale(locale);
                })[0];
                if (language) {
        ...
        if (languageCollection.length) {
            if (locale) {
                return languageCollection
                    .filter(function (t) {
                    return t.locale === locale ||
                        internal_1.Utils.getInexactLocale(t.locale) ===
                            internal_1.Utils.getInexactLocale(locale);
                })
