...
var Sequence = /** @class */ (function (_super) {
    ...
    Sequence.prototype.getCanvasIndexByLabel = function (label, foliated) {
        ...
        for (var i = 0; i < this.getTotalCanvases(); i++) {
            ...
            if (internal_1.LanguageMap.getValue(canvas.getLabel(), this.options.locale) === label) {
                return i;
            }
    ...
    Sequence.prototype.getLastCanvasLabel = function (alphanumeric) {
        for (var i = this.getTotalCanvases() - 1; i >= 0; i--) {
            var canvas = this.getCanvasByIndex(i);
            var label = (internal_1.LanguageMap.getValue(canvas.getLabel(), this.options.locale));
            if (alphanumeric) {
